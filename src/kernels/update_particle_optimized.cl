...
kernel void update_particle_optimized(global Particle_optimized *p, global float *f, int numForces, float deltaTime) {
	for (int j = 0; j < 2; ++j) {
		...
		p[i].position += (float4)(0.0f, 0.0f, -0.001f, 0.0f);
	}
}